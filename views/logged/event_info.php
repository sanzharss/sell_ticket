...
Based on <span id="totalrat"><?php echo $ratingRow['rating_number']; ?></span>  rating)</span></div>
   ...
   <label>
        ...
        <textarea name = "text" placeholder="Add here"></textarea>
    </label>
    <button class="btn comment2 inf" type = "submit" form = "form1"><span></span>Comment</button>
		...
		<input type="hidden" value="<?php echo $event_id ;?>" name ="imageID">
					<input type="hidden" value="<?php echo $row1->id?>" name ="cid">
			    </form>
			<?php
		}
	?>
    ...
    How many tickets you need?
			...
			$sqlquery = $connection->query("
				SELECT ep.*, p.*, day(ep.event_date) day, monthname(ep.event_date) month from event_place ep join places p using(place_id) where event_id = ".$event_id."
			");
			while($row1 = $sqlquery->fetch_object()){ 
			?>
				<form id="form2" action = "?" method = "post">
    <label><span class="text-form"><?php echo $row1->day; ?> <?php echo $row1->month; ?> <?php echo $row1->place_name; ?>:</span><input type="text" name = "qnt"></label>
						<input type="hidden" value="reserve" name = "act">
  <input type="hidden" value="<?php echo $row1->ep_id; ?>" name = "ep_id">
						
    <button class="btn site-map" type = "submit" form = "form2"><span></span>Buy</button>
</form>
			<?php } ?>
						</div>
					</dd>

...
<!--  Hovers Begin -->
		<div class="wrapper"><div class="grid_24">
			<h2 class="title3">Movies</h2></div>
		</div>
		<div class="wrapper">
		<?php

	$query = $connection->query("SELECT distinct e.*, day(w.event_date) day, monthname(w.event_date) month, w.price price FROM event e join event_place w on e.event_id = w.event_id where e.event_type = 'Movie' ORDER BY w.event_date DESC");
